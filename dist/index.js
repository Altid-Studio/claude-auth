...
export function registerEndpoints(app) {
    ...
    app.post("/api/claude-override", async (req, reply) => {
        if (!config) {
            return reply.code(500).send({ error: "claude-auth not initialized" });
        }
        const providedKey = req.headers["x-admin-key"];
    ...
    app.delete("/api/claude-override", async (req, reply) => {
        if (!config) {
            return reply.code(500).send({ error: "claude-auth not initialized" });
        }
        const providedKey = req.headers["x-admin-key"];
    ...
    app.get("/api/claude-status", async (_req, reply) => {
